...
library(ISOweek)
path <- "/Users/andrew/Desktop/Riff_Analytics_Internship/analyze-data/user_reports/Amy/"
```
...
# User Report for: Amy 'jMZgnpJrX1QwAN0oUkYNme9kD4b2'
...
g_w <- ggplot(weekoverall, aes(x = time, y = average_speaking_time) ) +
     ...
     geom_hline(yintercept = overall_avgs$avg_user_time, linetype='dashed', color='black', size=1) +
...
weekoverall <- weekoverall %>% filter(SU_affirmations<10) %>% 
  filter(SU_interruptions<.35 & SU_interruptions>=0) %>% 
  filter(SU_influences<12) %>% 
  filter(SU_interrupted>=0)
